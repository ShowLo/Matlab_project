function tonePlay = toneHarmonics(freq_sample,frequence,last_time)
%输入参数
%freq_sample：采样频率
%frequence：乐音频率
%last_time：乐音持续时间
%输出参数
%tonePlay：一段由若干个乐音组合且经过包络处理和添加了谐波分量而成的音乐片段
tonePlay = [];
for i = 1 : length(frequence)
    ...
    %添加二次谐波和三次谐波
    part_tone = (sin(2 * pi * frequence(i) * t) + 0.2 * sin(2 * pi * 2 * frequence(i) * t)...
                       + 0.3 * sin(2 * pi * 3 * frequence(i) * t)) .* envelope;
